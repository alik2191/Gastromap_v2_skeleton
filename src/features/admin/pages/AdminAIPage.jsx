...
import {
    ...
    Save, Play, Pause, RefreshCw, Sliders, Brain, ChevronRight
} from 'lucide-react'
import { cn } from '@/lib/utils'

const AgentCard = ({ name, role, isActive, icon: Icon, color, description }) => (
    <div className="bg-white dark:bg-slate-900 p-6 lg:p-10 rounded-[28px] lg:rounded-[48px] border border-slate-100 dark:border-slate-800 shadow-sm relative overflow-hidden group hover:border-indigo-500/20 transition-all duration-500">
        ...
        <div className="relative z-10">
            ...
            <div className="flex gap-2">
                ...
                <button className="p-3 lg:p-4 rounded-xl lg:rounded-2xl bg-slate-50 dark:bg-slate-800 text-slate-400 hover:text-slate-600 dark:hover:text-white hover:bg-slate-100 transition-all shadow-inner shrink-0">
...
const AdminAIPage = () => {
    const [guideTemp, setGuideTemp] = useState(0.7)
    const [assistantTemp, setAssistantTemp] = useState(0.4)
    ...
    return (
        <div className="space-y-6 lg:space-y-8 pb-10 font-sans">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-5">
                <div>
                    <h1 className="text-xl lg:text-2xl font-bold text-slate-900 dark:text-white leading-tight">Управление ИИ</h1>
                    <p className="text-slate-500 dark:text-slate-400 font-medium mt-0.5 text-xs lg:text-sm">Конфигурация интеллектуальных агентов.</p>
                </div>
                <button className="w-full sm:w-auto flex items-center justify-center gap-2 px-6 py-2.5 bg-indigo-600 text-white rounded-xl font-bold text-[10px] lg:text-[11px] uppercase tracking-widest shadow-lg shadow-indigo-500/20 active:scale-95 transition-all">
                    <Save size={18} />
                    Сохранить
                </button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 lg:gap-8">
                <AgentCard
                    ...
                    description="Отвечает на вопросы пользователей, подбирает локации на основе предпочтений и помогает планировать маршруты."
                />
                <AgentCard
                    name="AI Assistant"
                    role="Admin Helper"
                    ...
                    description="Анализирует тренды, следит за модерацией отзывов, оповещает о критических событиях и массовых операциях."
                />
            </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 lg:gap-8">
                <div className="xl:col-span-2 bg-white dark:bg-slate-900 rounded-[28px] lg:rounded-[48px] border border-slate-100 dark:border-slate-800 shadow-sm p-6 lg:p-10">
                    <div className="flex items-center gap-3 mb-10 pl-1">
                        ...
                        <h2 className="text-lg lg:text-xl font-bold text-slate-900 dark:text-white">Параметры моделей</h2>
                    </div>
                    ...
                    <div className="space-y-12">
                        ...
                        <div className="space-y-6 lg:space-y-8 bg-slate-50/50 dark:bg-slate-800/30 p-5 lg:p-8 rounded-[24px] lg:rounded-[32px] border border-slate-100 dark:border-slate-800 shadow-inner">
                            <div className="flex justify-between items-start">
                                <div className="min-w-0 flex-1 pr-4">
                                    <h4 className="text-sm lg:text-lg font-bold text-slate-900 dark:text-white leading-tight truncate">Креативность AI Guide</h4>
                                    <p className="text-[10px] lg:text-sm font-medium text-slate-400 dark:text-slate-500 mt-1.5 leading-relaxed truncate">Разнообразие и непредсказуемость ответов.</p>
                                </div>
                        ...
                        <div className="space-y-6 lg:space-y-8 bg-slate-50/50 dark:bg-slate-800/30 p-5 lg:p-8 rounded-[24px] lg:rounded-[32px] border border-slate-100 dark:border-slate-800 shadow-inner">
                            <div className="flex justify-between items-start">
                                <div className="min-w-0 flex-1 pr-4">
                                    <h4 className="text-sm lg:text-lg font-bold text-slate-900 dark:text-white leading-tight truncate">Точность AI Assistant</h4>
                                    <p className="text-[10px] lg:text-sm font-medium text-slate-400 dark:text-slate-500 mt-1.5 leading-relaxed truncate">Строгость и точность аналитических ответов.</p>
                                </div>
                ...
                <div className="bg-white dark:bg-slate-900 rounded-[28px] lg:rounded-[48px] border border-slate-100 dark:border-slate-800 shadow-sm p-6 lg:p-8 flex flex-col">
                    <h2 className="text-[9px] lg:text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500 mb-8 lg:mb-10 pl-1">Статус систем</h2>
                    <div className="space-y-8 flex-1 pl-1">
                        ...
                        <div className="flex items-center justify-between group">
                            <div className="flex items-center gap-4">
                                ...
                                <span className="text-[13px] lg:text-sm font-bold text-slate-600 dark:text-slate-400 group-hover:text-slate-900 dark:group-hover:text-white transition-colors">Usage</span>
                            </div>
                            <span className="text-[13px] lg:text-sm font-bold text-slate-900 dark:text-white">42%</span>
                        </div>
                        <div className="flex items-center justify-between group">
                            <div className="flex items-center gap-4">
                                ...
                                <span className="text-[13px] lg:text-sm font-bold text-slate-600 dark:text-slate-400 group-hover:text-slate-900 dark:group-hover:text-white transition-colors">Synced</span>
                            </div>
                            <span className="text-[13px] lg:text-sm font-bold text-slate-400 dark:text-slate-600">Now</span>
                        </div>
                    </div>
                    ...
                    <div className="mt-10 p-5 lg:p-6 rounded-[24px] lg:rounded-[32px] bg-slate-50 dark:bg-slate-800/20 border border-slate-100 dark:border-slate-800 relative overflow-hidden group">
                        ...
                        <p className="text-[11px] lg:text-sm font-medium text-slate-600 dark:text-slate-400 leading-relaxed italic relative z-10">
                            "Все системы стабильны. AI Assistant обработал 150 запросов за последний час."
                        </p>
                    </div>
                </div>
            </div>
